...
namespace Rivet {
  ...
  class BTAG_JET_TRACK_SEL : public Analysis {
  public:

    /// Constructor
    BTAG_JET_TRACK_SEL()
      : Analysis("BTAG_JET_TRACK_SEL")
    {
  ...
  public:
    ...
    void init() {
      ...
      for(int jetflav=0;jetflav<njetflavors;jetflav++){
        ...
        _h_SecVertexPVD[jetflav]                = bookHisto1D("S_SecVertexPVD_"+jetflavorname[jetflav]+"Jets", 40, 0.,40.);
        
        _h_MuonPtRel[jetflav]                   = bookHisto1D("T_MuonPtRel_"+jetflavorname[jetflav]+"Jets", 40, 0., 20.);
        _h_MuonJetDr[jetflav]                   = bookHisto1D("U_MuonJetDr_"+jetflavorname[jetflav]+"Jets", 70, 0., .7);     
      }
    }
    ...
    void analyze(const Event& event) {
      ...
      Particles particles = applyProjection<VisibleFinalState>(event,"VFS").particles();
      ...
      const Jets jets = applyProjection<FastJets>(event, "Jets").jetsByPt(5.*GeV);
      ...
      foreach (const Jet& j, jets) {
        ...
        cout<<"Jet pT: "<< j.momentum().pT() <<" eta: "<< j.eta() <<endl;
        ...
        //Get Jet Flavor
        int jetflavorID = -1;
        if(j.bTagged()) jetflavorID = 3;
        else if(j.cTagged()) jetflavorID = 2;
        else jetflavorID = 1;
        
        if(j.bTagged()) cout<<"-> Tagged!"<<endl;
        ...
        foreach (const Particle& cp, cparticles){
          ...
          if(particlevertex.x() != particlevertex.x() || particlevertex.y() != particlevertex.y() || particlevertex.z() != particlevertex.z()) continue;
        ...
        for(size_t iv=0;iv<vertex_position.size();++iv){
          ...
          if(dr_sv_jet>dr_sv_jet_max) continue;
          
          const float d_sv_pv = (vertex_position[iv]-pvertex).mod();
          if(d_sv_pv<pvertexcutoff) continue;
          if(d_sv_pv>d_sv_pv_max) continue;
          
          const float m_sv = vertex_fourmomentum[iv].mass();
          if(m_sv>m_sv_max) continue;
          if(m_sv>(1-m_kaon_window)*m_kaon && m_sv<(1+m_kaon_window)*m_kaon) continue;
          
          ++nsv;
          
          _h_SecVertexMass[0]->fill(m_sv, weight);
          _h_SecVertexMass[jetflavorID]->fill(m_sv, weight);
          
          _h_SecVertexJetDr[0]->fill(dr_sv_jet, weight);
          _h_SecVertexJetDr[jetflavorID]->fill(dr_sv_jet, weight);
          
          _h_SecVertexPVD[0]->fill(d_sv_pv, weight);
          _h_SecVertexPVD[jetflavorID]->fill(d_sv_pv, weight);
	      }
    ...
    void VertexFinder(vector<ThreeVector>& vertex_position, vector<FourMomentum>& vertex_fourmomentum, vector<float>& sumpt2, const Particles praticles, const float cutoffvalue = 0.001){
      ...
      for(Particles::const_iterator itp = praticles.begin();itp!=praticles.end();++itp){
        ...
        if(HEPMCparticlevertex.x() != HEPMCparticlevertex.x() || HEPMCparticlevertex.y() != HEPMCparticlevertex.y() || HEPMCparticlevertex.z() != HEPMCparticlevertex.z()) continue;
  ...
  private:
    ...
    //Secondary Vertex Selection parameters;
    const float dr_sv_jet_max = .5;
    const float d_sv_pv_max   = 2.5*cm;
    const float m_sv_max      = 6.5*GeV;
    const float m_kaon        = 497.6*MeV;
    const float m_kaon_window = 0.2;
  ...
  DECLARE_RIVET_PLUGIN(BTAG_JET_TRACK_SEL);
